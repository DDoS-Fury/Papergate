...
\node[op, below=0.7cm of CAT, text width=11cm] (GNN)
  {\textbf{GraphAttentionEmbedding} -- 3 hop su vicinato temporale (\texttt{neighbor\_size}=30);\\
   edge\_attr $= [\,$time\_enc$(\Delta t_{hist})\,\|\,$hist\_msg$\,] = [32{+}6] = 38$\\
...
\node[head, below=0.95cm of GNN, xshift=-3.3cm, text width=6.3cm] (FEAT)
  {\textbf{Feature head — LinkPredictor (MLP)}\\
   input $[649]=$ z\_src$[256]\,\|\,$z\_dst$[256]\,\|\,$msg$[6]$\\
   $\|\,$fwh\_src$[32]\,\|\,$fwh\_dst$[32]\,\|\,\Delta t[32]\,\|\,\Delta t_{src}[32]\,\|\,$hist$[3]$\\
   Linear$(649{\to}256)$ + ReLU\\
...
La validazione di questo approccio sul dataset reale pubblico \textbf{LANL Comprehensive Multi-Source} (etichette red-team) ha restituito metriche eccellenti: il modello ha mantenuto il \textbf{100\% del recall} sui movimenti laterali, facendo crollare i falsi positivi (FPR) dal 1.40\% allo \textbf{0.38\%}, con una AUC sul laterale che ha raggiunto \textbf{0.9981}. Il mapping (\texttt{tests/datasets/lanl\_auth.py}) tiene le colonne di allarme \emph{pulite} — le etichette red-team finiscono solo in $y$/\textit{types}, mai nelle feature — così la rule baseline resta cieca al laterale e il discriminante è puramente temporale/relazionale (test onesto, nessun data leak). \emph{Caveat di campionamento}: la valutazione è su una finestra temporale mirata che contiene poche decine di eventi red-team (pochi nel solo test set); i valori molto alti riflettono che il laterale di LANL — coppie host genuinamente nuove — è più separabile del caso sintetico, deliberatamente costruito perché il laterale sia feature-identico al benigno. Validità esterna aggiuntiva (DARPA OpTC, CIC-IDS) resta lavoro futuro.
