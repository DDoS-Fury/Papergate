...
\node[note, text width=3.2cm, left=0.7cm of HOP2] (EDGE)
...
\draw[res] (HOP1.east) -- ++(0.45,0) |- (HOP2.east);
...
\node[op] (SUM) at (HOP3 |- BOT) {logit $=$ feat\_logit $+$ struct\_logit};
...
\begin{scope}[on background layer]
  \node[gaebox, fit=(HOP1)(HOP2)(HOP3)(EDGE)(GAER),
        label={[lbl, anchor=south west]north west:GraphAttentionEmbedding --- 3 hop, \texttt{neighbor\_size}=30}] {};
...
Il processo di ottimizzazione (guidato dall'algoritmo AdamW) somma tre termini auto-supervisati. La componente dominante è una loss di ranking InfoNCE, allineata all'Average Precision, affiancata da due ancoraggi BCE:
...
\begin{enumerate}
    ...
    \item \textit{Lateral Movement}: l'accesso è formalmente autorizzato a livello di regole OPA, ma \textit{non abituale}. Avviene tipicamente nel ``pivoting''. Essendo indistinguibile sintatticamente dal traffico benigno, la classificazione ricade quasi del tutto sull'architettura GNN.
\end{enumerate}
