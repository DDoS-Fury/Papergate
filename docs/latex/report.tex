...
\section{Architettura del nodo \textit{Configuration}}
...
\subsection{Topologia: la catena causale a 5 archi}
...
% ============================ RISULTATI v4 ============================
\section{Risultati: il contributo del nodo config}
\label{sec:v4results}

La valutazione segue lo stesso protocollo del capitolo precedente (replay per-evento attraverso l'esatto codice di serving, split cronologico 70/10/20, soglia cost-sensitive instradata). Si riportano due confronti complementari: (i) il \emph{deployable} v3 vs v4 all'\textit{operating point} pubblicato (singola run, 200\,000 eventi / 15 epoche, \texttt{seed}=42), e (ii) la \emph{validazione mirata} del nodo config — un'ablazione controllata \emph{a parità di dati} su seed multipli.

\subsection{Confronto del modello pubblicato (v3 $\rightarrow$ v4)}

La Tabella~\ref{tab:v3v4} confronta l'artefatto deployabile prima e dopo l'introduzione del nodo config, al medesimo \textit{operating point} (decisione cost-sensitive instradata sul segnale). Il guadagno più rilevante è sul \textbf{movimento laterale}: il \textit{recall} operativo passa da $\sim$25.5\% a $\sim$59.9\% (più che raddoppiato), con AP $0.464\rightarrow0.614$ e AUC $0.894\rightarrow0.936$, a fronte di un FPR benigno quasi invariato ($\sim$4.8\% $\rightarrow$ $\sim$5.6\%). È il comportamento atteso per costruzione: \textit{config}$\rightarrow$\textit{device} cattura il ``tool nuovo su device noto'' e \textit{config}$\rightarrow$\textit{user} il ``client che quell'utente non usa abitualmente'', cioè i \textit{tell} del laterale e del furto di credenziali.
...
\hline
AUC aggregata                    & 0.952 & \textbf{0.964} & $+0.012$ \\
AP aggregata                     & 0.896 & \textbf{0.905} & $+0.009$ \\
Recall aggregata (instradata)    & 0.709 & \textbf{0.835} & $+0.126$ \\
\hline
Lateral --- AUC                  & 0.894 & \textbf{0.936} & $+0.042$ \\
Lateral --- AP                   & 0.464 & \textbf{0.614} & $+0.150$ \\
Lateral --- Recall (instradata)  & 0.255 & \textbf{0.599} & $+0.344$ \\
Lateral --- Recall (@1\%FPR)     & 0.141 & 0.152 & $+0.011$ \\
\hline
Policy --- AUC / Recall@thr      & $\sim$0.95 / -- & 0.967 / 0.941 & $\sim$ \\
Contextual --- AUC / Recall@thr  & $\sim$0.99 / -- & 0.990 / 0.970 & $\sim$ \\
\hline
FPR benigno (instradato)         & $\sim$0.048 & $\sim$0.056 & $+0.008$ \\
\hline
\end{tabular}
\caption{Confronto del modello \emph{deployabile} v3 vs v4 sul medesimo test set sintetico (replay per-evento, \texttt{num\_events}=200\,000, \texttt{seed}=42, decisione cost-sensitive instradata). \emph{Nota metodologica}: valori da \emph{singola} run (training GPU non-deterministico, $\pm0.01$--$0.03$ per run sul lateral AUC); il segnale robusto è il salto di \textit{recall} operativo del laterale ($+0.344$), ben oltre il rumore single-run. Le classi \textit{policy} (OPA-owned) e \textit{contextual} (rule-trivial) restano invariate per costruzione. Il confronto con le baseline non-relazionali (Tab.~\ref{tab:baselines}) è immutato: nessuna baseline modella il nodo config, quindi il vantaggio di v4 si \emph{somma} a quello già documentato del TGN sul segnale temporale.}
\label{tab:v3v4}
\end{table}

\subsection{Validazione mirata: il furto di credenziali}
...
\subsection{Sweep architetturale}
\label{sec:archsweep}
...
\paragraph{Limiti e costo non misurato.} Il guadagno sulle metriche di detection va soppesato con un costo che esse \emph{non} catturano: collassando i dispositivi senza TPM si perde l'\emph{attribuzione per-macchina} (tracciabilità forense, capacità di isolare un singolo endpoint compromesso, audit). Inoltre il confronto è distribuzionale e su stream sintetico: il $\Delta$ sull'AUC laterale è entro il rumore e la quota di guadagno dovuta alla riduzione di varianza non è separabile da un eventuale guadagno ``vero''. Per questi motivi il \textit{deployable} mantiene il \textit{default} a \textit{keying} per-cookie (\texttt{guest\_device\_fallback=False}); il collasso su \texttt{dev:guest} resta una politica attivabile e un'evidenza utile sul fatto che, su questo stream, l'identità fine del dispositivo \emph{non} è il portatore del segnale.
...
\toprule
\textbf{Modello / variante} & \textbf{AUC agg} & \textbf{AP agg} & \textbf{Lat.\ AUC} & \textbf{Lat.\ AP} & \textbf{Lat.\ Recall} \\
\midrule
\multicolumn{6}{l}{\textit{Pannello A — stream standard, single run (200k ev., seed 42), soglia globale 1\% FPR}} \\
\addlinespace[2pt]
\textbf{TGN} (v3, deployable)            & \textbf{0.947} & \textbf{0.870} & \textbf{0.900} & ---   & 0.130 \\
GNN non temporale (ablation)             & 0.574 & 0.569 & 0.468 & ---   & 0.139$^{\dagger}$ \\
One-Class SVM                            & 0.845 & 0.793 & 0.632 & ---   & 0.047 \\
Isolation Forest                         & 0.775 & 0.628 & 0.639 & ---   & 0.010 \\
XGBoost (supervisionato)$^{\ddagger}$    & 0.977 & ---   & 0.942 & ---   & ---   \\
\midrule
\multicolumn{6}{l}{\textit{Pannello B — stream standard, single run, decisione cost-sensitive instradata}} \\
\addlinespace[2pt]
v3 (4 nodi)                              & 0.952 & 0.896 & 0.894 & 0.464 & 0.255 \\
\textbf{v4} (nodo config)                & \textbf{0.964} & \textbf{0.905} & \textbf{0.936} & \textbf{0.614} & \textbf{0.599} \\
\midrule
\multicolumn{6}{l}{\textit{Pannello C — stream theft-rich, 3 seed (media), ablazioni controllate (\texttt{save=False})}} \\
\addlinespace[2pt]
v4 (nodo config)                         & 0.958 & ---   & 0.919 & ---   & 0.469 \\
$\approx$v3 (ablazione config)           & 0.956 & ---   & 0.913 & ---   & 0.334 \\
v4 $+$ \texttt{dev:guest}                & 0.961 & ---   & 0.911 & 0.632 & 0.488 \\
\bottomrule
\end{tabular}
\caption{Quadro riepilogativo delle prestazioni di tutti i modelli e varianti osservati. \textbf{Solo i confronti intra-pannello sono validi} (protocolli di valutazione diversi). \textit{Lat.\ Recall} è a soglia globale 1\% FPR nel Pannello~A e a decisione instradata nei Pannelli~B e~C. Nel Pannello~A il \textbf{TGN} coincide col \textit{deployable} v3: i valori differiscono lievemente da quelli del Pannello~B per la diversa regola di decisione (soglia globale vs instradata) e per la varianza single-run. I valori di Pannello~C sono medie su 3 \texttt{seed} su uno stream \emph{theft-rich} (incidenti più frequenti, non appaiati evento-per-evento), perciò non comparabili in assoluto con i Pannelli~A/B. $^{\dagger}$ Il recall della GNN statica è \emph{spurio} (AUC$\approx$caso a soglia permissiva, non segnale reale). $^{\ddagger}$ XGBoost è \emph{supervisionato} (vede le etichette): upper-bound di riferimento, fuori dal paradigma non supervisionato del TGN. Lo \textit{sweep} architetturale (Tab.~\ref{tab:archsweep}) conferma inoltre che nessun aumento di capacità migliora il movimento laterale. Sorgenti: Tab.~\ref{tab:baselines} (A), Tab.~\ref{tab:v3v4} (B), Tab.~\ref{tab:theft} e~\ref{tab:guestdev} (C).}
\label{tab:summary}
\end{table}
