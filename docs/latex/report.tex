...
\node[inp, right=0.8cm of ZMEM] (NF) {nf $[16]$\\\textit{tier/risk/internal}};
...
\node[head, below=0.95cm of GNN, xshift=-3.3cm, text width=6.3cm] (FEAT)
  {\textbf{Feature head — LinkPredictor (MLP)}\\
   input $[653]=$ z\_src$[256]\,\|\,$z\_dst$[256]\,\|\,$msg$[7]$\\
   $\|\,$fwh\_src$[32]\,\|\,$fwh\_dst$[32]\,\|\,\Delta t[32]\,\|\,\Delta t_{src}[32]\,\|\,$hist$[6]$\\
   Linear$(653{\to}256)$ + ReLU\\
...
La validazione di questo approccio in modalità \textit{Device-Centric} sul dataset reale pubblico \textbf{LANL Comprehensive Multi-Source} (etichette red-team) ha certificato la robustezza dell'infrastruttura. Effettuando uno split cronologico rigoroso (addestramento sul primo 30\% ed espansione del test escluso da data leakage sul restante 60\%), è stato possibile analizzare l'intero periodo operativo del Red Team, abbracciando centinaia di attacchi furtivi confermati. Il modello scala mantenendo metriche di altissimo livello, tra cui un \textbf{Recall superiore al 73\%} sui movimenti laterali, con una \textbf{AUC Aggregata di 0.8824} e falsi positivi (FPR) contenuti al \textbf{2.18\%}.
