...
% ==================== SCORE() ====================
\node[input, below=1.5cm of EMBED_GROUP, xshift=-3cm] (Z_IN) {z};
\node[input, right=1cm of Z_IN] (CUR) {src\_local, dst\_local, cur\_msg};
\node[input, right=1cm of CUR] (NF_IN) {nf};
\node[input, right=0.5cm of NF_IN] (HIDX_IN) {h\_idx};

\node[box, below=0.6cm of HIDX_IN] (HE2) {nn.Embedding\\(he)};
\draw[edge] (HIDX_IN) -- (HE2);

\node[box, below=0.6cm of NF_IN, xshift=1cm] (FWH) {feat\_with\_hash\\[ nf $\parallel$ he ]};
\draw[edge] (NF_IN) |- (FWH);
\draw[edge] (HE2) |- (FWH);

\node[box, below=2.5cm of Z_IN] (STRUCT) {Structural Head\\struct\_scale $\cdot$ $\sum$ (norm(proj(z\_src)) $\odot$ norm(proj(z\_dst)))\\$\rightarrow$ struct\_logit};

\node[box, below=2.5cm of CUR, xshift=2.5cm] (LINK) {LinkPredictor (MLP)\\[ z\_src $\parallel$ z\_dst $\parallel$ cur\_msg $\parallel$ fwh\_src $\parallel$ fwh\_dst ]\\$\rightarrow$ feat\_logit};

% Orthogonal routing into LINK
\draw[edge] (FWH.south) |- ++(0, -0.4) -| (LINK.50);

% Orthogonal routing for CUR
\draw[edge] (CUR.south) -- ++(0,-0.6) -| (STRUCT.50);
\draw[edge] (CUR.south) -- ++(0,-0.6) -| (LINK.110);

% Orthogonal routing for Z_IN
\draw[edge] (Z_IN.south) -- (STRUCT.north);
\draw[edge] (Z_IN.south) -- ++(0,-1.2) -| (LINK.150);

\node[box, below=1cm of LINK, xshift=-3.5cm] (SUM) {logit = feat\_logit + struct\_logit};
\draw[edge] (LINK) |- (SUM);
...
\draw[dashededge] (FEAT.0) -- ++(5.5,0) |- ++(0,-6.5) -| (NF_IN.north);
\draw[dashededge] (NID.0) -- ++(7.5,0) |- ++(0,-8) -| (HIDX_IN.north);
