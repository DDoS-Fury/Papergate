...
with destination novelty. Introducing legitimate non-habitual access forces the detector
to discriminate based on relational history and temporal sequences rather than target novelty alone.
...
Dispersion is reported as the sample standard deviation over the three seeds, one run
per seed, with $\text{ddof}=1$; paired comparisons use per-seed differences evaluated with a
Wilcoxon signed-rank test and bootstrap confidence intervals. With three seeds, the signed-rank
...
Five baselines are evaluated alongside the model: Isolation Forest, One-Class SVM,
XGBoost, a static graph neural network (identical graph structure without temporal memory),
and a standard two-node bipartite TGN (\textit{user} $\rightarrow$ \textit{resource}).
...
The static GNN isolates the role of recurrent temporal state, while the two-node TGN isolates
