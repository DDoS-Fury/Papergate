...
A survey of public cybersecurity datasets highlights two systemic telemetry gaps:
...
intrusion across five workstations and one domain controller, published as Zeek JSON logs
...
Because PicoDomain contains only \PicoNlateral{} lateral events across eight source addresses
and lacks authorization policies, it serves as an empirical case study rather than a benchmark.
However, it confirms that all five node types map directly to real enterprise Zeek telemetry.
...
benchmark, reflecting the limited sample size and small entity population, the model
maintains inductive ranking capability on real network traces.
...
Equipping multi-source corpora like AIT with explicit lateral-movement labels is necessary
to benchmark fine-grained Zero Trust graph architectures at scale.
