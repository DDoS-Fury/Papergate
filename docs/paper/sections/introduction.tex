...
a temporal graph is an established formulation, with Euler~\cite{euler} and
Argus~\cite{argus} as reference systems and the LANL authentication
...
\begin{enumerate}
  ...
  \item \textbf{An external policy-driven commit gate.} To mitigate self-poisoning
  ...
  \item \textbf{A rigorous leakage audit for synthetic benchmarks.} To prevent
  unintended feature shortcuts common in synthetic benchmark generation, we develop
  a regression audit that bounds single-feature separability, forbids exact-value
  fingerprints, and equalises destination marginals across benign and anomalous classes
  (Section~\ref{sec:setup}).

  \item \textbf{An external-validity study on real enterprise telemetry.} We survey
  public cybersecurity datasets and identify the absence of corpora uniting user identity
  and TLS fingerprints. On PicoDomain~\cite{picodomain}, the only corpus providing both,
  we implement the schema adapter, quantify session-binding coverage, and evaluate
  transferability (Section~\ref{sec:external}).
\end{enumerate}
