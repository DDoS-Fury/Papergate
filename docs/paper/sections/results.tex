...
Furthermore, benchmark corpora like LANL omit client fingerprints entirely; evaluating
our architecture on such data requires ablating the configuration node, as examined in
Section~\ref{sec:external}.

Two additional operational distinctions are significant. Snapshot-based models discretize
...
\begin{table*}[t]
\centering
\caption{Synthetic stream evaluation (\PanelAsettings). All baselines receive the identical
tabular features, causal history counters, and precursor priors as the TGN to isolate the
effect of temporal graph modeling. $^{\ddagger}$ Supervised upper-bound reference (trained
...
\textbf{TGN (5-node, this work)} & \textbf{\AggAucTGN} & \textbf{\AggApTGN} & \textbf{\LatAucTGN} & \textbf{\LatRecTGN} & \textbf{\AggRecTGN} \\
\midrule
...
The critical baseline comparison is against the single-feature floor (Table~\ref{tab:floor},
...
More crucially, as demonstrated in Section~\ref{sec:approach} and Table~\ref{tab:panelb},
...
On our de-leaked benchmark, lateral movement is strictly bounded below by the single-feature
...
demonstrating effective relational anomaly detection without relying on artificial shortcuts.
...
isolated structural cosine heads) involve significant combinatorial sweeps across seeds.
We therefore restrict our verified empirical claims to the five-node causal chain, the
