...
both regenerated from HEAD and the TGN
...
\toprule
& \textit{Euler}~\cite{euler} & \textit{Argus}~\cite{argus} & \textit{LMDetect}~\cite{lmdetect} & GFM~\cite{gfm} & CloudTrail~\cite{cloudtrailtgn} & Static GNN & IF / OC-SVM & \textit{This work} \\
\midrule
Substrate            & auth log & auth log & auth log & auth log & cloud log & ZTA stream & ZTA stream & \textit{ZTA requests} \\
Node types           & host & host & host, user & host, user & acct., res. & 5 & tabular & \textit{5, incl.\ config} \\
Client fingerprint node & no & no & no & no & no & yes & --- & \textit{yes (JA3)} \\
Temporal granularity & snapshots & snapshots & subgraphs & snapshots & per event & none & none & \textit{per event} \\
Recurrent entity memory & via RNN & yes & no & no & yes & no & no & \textit{yes (GRU)} \\
Inductive on new entities & partial & partial & partial & yes & yes & yes & yes & \textit{yes (hashed id.)} \\
Learning regime      & one-class & one-class & supervised & pretrained & self-sup. & one-class & one-class & \textit{one-class} \\
Policy-gated memory commit & no & no & no & no & no & n/a & n/a & \textit{yes} \\
Credential reuse evaluable & no & no & no & no & no & yes & yes & \textit{yes} \\
Serving artifact & --- & --- & --- & --- & --- & --- & --- & \textit{yes (HTTP)} \\
Evaluation corpus & LANL, OpTC & LANL, OpTC & LANL, OpTC & LANL, OpTC & private & \multicolumn{3}{c}{synthetic ZTA stream $+$ PicoDomain} \\
\bottomrule
\end{tabular}
\caption{Positioning relative to the temporal-graph detection literature.
``Per event'' denotes events scored sequentially upon arrival rather than aggregated
into windowed snapshots; ``config'' is the client TLS configuration (JA3). CloudTrail: the self-supervised TGN over AWS CloudTrail
logs~\cite{cloudtrailtgn}, evaluated on five private organizations.}
...
or, in the cloud case, over a single account-to-resource relation. Consequently, they cannot
capture adversaries replaying valid credentials from novel client configurations as a
structural anomaly, as client fingerprints are omitted from their graph schema.
Benchmark corpora like LANL also omit client fingerprints entirely; evaluating the
architecture on such data requires ablating the configuration node (Section~\ref{sec:external}).

Per-event streaming scoring does not by itself separate this protocol from prior work: a
continuous-time TGN already scores each cloud API event on arrival~\cite{cloudtrailtgn}, whereas
the snapshot models discretize the stream into fixed windows that a PDP cannot wait for. The
operational distinction is the commit gate. The CloudTrail TGN updates its memory with the
events it observes at inference time, with no gating of those updates described, and snapshot
evaluations do not model a commit decision at all; here the external policy engine decides
which requests enter the detector's baseline. Every TGN result in this paper runs under that
gate; the effect of the gate against an ungated, commit-everything baseline is not yet
quantified.
...
The two-node bipartite TGN (\textit{user} $\rightarrow$ \textit{resource}) focuses all memory
...
outperforms the two-node baseline (\LatAucTGN{} vs \LatAucTGNii{} lateral AUC, a \LatAucDeltaTGNii{} margin).
...
floor of $\FloorLatPost$ AUC. The 5-node TGN exceeds this floor, achieving \LatAucTGN{}
lateral AUC and \LatApTGN{} AP under benign-only one-class training.
Fig.~\ref{fig:perf_bars} reports the per-class AUC, Average Precision, and operating
...
\bottomrule
\end{tabular}
\caption{Four-node chain versus 5-node chain with the configuration node, same
stream (\PanelBsettings).}
\label{tab:panelb}
\end{table}

Table~\ref{tab:panelb} isolates the configuration node by ablating it on the same
stream under routed evaluation. Adding the configuration node improves both ranking and
detection metrics: aggregate AUC increases by \BaggAucD{} (to \BaggAucIV), lateral AUC by
\BlatAucD{} (from \BlatAucIII{} to \BlatAucIV), and lateral Average Precision by \BlatApD.
Operating recall on lateral movement reaches \BlatRecIV{} at an operating benign
false-positive rate of \BfprIV. In credential
...
expressiveness: on a four-node chain, credential reuse presents no anomalous edge. Adding
