...
\subsection{The 5-node causal chain}
...
\end{tikzpicture}
\caption{A single request, 5 edges. Dashed binding edges carry an all-zero message; only the
...
\node[st, right=6mm of req] (dec)
  {\textbf{5-edge}\\\textbf{decomposition}\\[1pt]\textit{4 bindings}\\\textit{+ 1 access}};
...
\node[st, left=8mm of fh, yshift=-5mm] (sc)
  {$a=-(\text{logit}_f{+}\text{logit}_s)$\\[1pt]$+$ \textbf{precursor}\\$\beta\,2^{-\Delta t/\tau}$\\[1pt]$s=\sigma(a)$};
\node[st, left=6mm of sc] (thr)
  {\textbf{threshold}\\[1pt]cost-sensitive\\(clean FPR cap)};
...
\end{tikzpicture}
\caption{Model architecture. The 5 edges of a single request are scored independently and
...
An access request spans contextual entities beyond the principal and target resource.
Representing it solely as $u \rightarrow r$ (the standard authentication-graph formulation)
omits the client execution context. Five edges are emitted per request (Fig.~\ref{fig:chain}): the four bindings
...
\textit{config}\,$\rightarrow$\,\textit{user}, and a dataset without client fingerprints
and device attestation (such as the LANL auth log) degrades to the single
\textit{user}\,$\rightarrow$\,\textit{resource} access edge. The event score is the maximum over the edges present, so a single anomalous binding
...
The first is a \textit{hashed identity}: a deterministic BLAKE2b hash of the external entity
...
The concatenation is passed through a 3-hop graph attention encoder over the entity's
...
The logit is the sum of the two heads. A \textit{feature head}, parameterized as an MLP over both
endpoint embeddings, the current message, static attributes, recency encodings, and history
counters, captures tabular signals and temporal novelty for lateral movement. A \textit{structural head},
...
For each edge, the counters $[\log(1{+}n_{\text{pair}}), \log(1{+}n_{\text{src}}),
n_{\text{pair}}/(n_{\text{src}}{+}1)]$ are computed, reflecting historical occurrences of the exact pair,
...
receive the same counter statistics (consumed as a flat per-event vector for the device
actor) to isolate the contribution of temporal graph modeling from tabular feature
engineering.
...
To incorporate this temporal context without poisoning graph state, a lightweight
per-entity alert state is maintained external to the TGN memory, and an exponentially
decaying prior $\beta\,2^{-\Delta t/\tau}$ is added to the anomaly logit of subsequent events
from that entity. The prior is additive on the logit because it is a prior on the odds: it
moves every event by the same evidence in nats wherever it sits on the curve, and being
bounded by $\beta$ it cannot drive a score to saturation on its own. Scoring is carried in
logit space for the same reason, as the probability $1-\sigma(\cdot)$ saturates to exactly
$1$ in single precision below a logit of $-16$, which ties the head of the ranking together
and leaves the prior no room above it.

The prior arms on a threshold of its own, lower than the decision threshold. Reconnaissance
sits below the decision threshold by construction, since otherwise it would already be an
alert; arming on the decision threshold would therefore let only the sensor flag arm the
chain. The arming threshold is set to the label-free clean-FPR quantile
(Section~\ref{sec:calib}); arming feeds the prior alone, while the trust feature and the
reported decision keep using the decision threshold.

The decay constant $\tau$ is set from the measured delay between reconnaissance and the
first lateral event on the same host. The two transitions of the kill chain do not share a
timescale: a $\tau$ that maximises lateral-movement detection degrades credential-theft
detection and the reverse, so a single decay constant is a compromise rather than a tuned
optimum. This mechanism operates as an inference-time prior outside the trained parameters,
as benign-only training would treat sensor flags as inactive.

\subsection{Threshold calibration}
\label{sec:calib}

The primary threshold (persisted with the model and used by serving) is the
cost-sensitive threshold, which calibrates operating recall against labeled lateral events in
the validation window. A label-free variant, a quantile of the signal-clean benign score
distribution at a target FPR, is recorded in the calibration metadata for deployments without
red-team labels. Both are computed on a validation replay using the identical commit gate
protocol as testing, via a two-pass fixed point with runtime snapshot and restoration between
passes, preventing memory state contamination during threshold estimation.
...
Inference is exposed as an HTTP service. Per request the flow is: map the 5 keys to
...
(\prelimnote, Fig.~\ref{fig:latency_cdf}), each request performing 5 neighbourhood expansions and 5 encoder
forward passes.
...
$\max|\Delta| = \ParityDelta$ across both the 5-edge chain and the bipartite baseline path.
