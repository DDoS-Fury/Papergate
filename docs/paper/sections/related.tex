...
authentication events is an established approach. \textbf{Euler}~\cite{euler} composes
...
treated as a standard baseline in subsequent literature. \textbf{LMDetect}~\cite{lmdetect}
...
We evaluate LANL in Section~\ref{sec:external} as an architectural ablation
(the configuration node removed) rather than as a direct peer comparison.

\subsection{Temporal graph learning}

We build on the Temporal Graph Network of Rossi et al.~\cite{tgn}, which
...
against the snapshot systems above is temporal granularity. Euler and snapshot-based
methods discretise the stream into fixed windows and encode snapshot sequences;
...
a requirement we enforce via the external commit gate (Section~\ref{sec:problem}).
...
\textbf{TrustGraph}~\cite{trustgraph} builds a heterogeneous graph from windowed
...
into closed-loop policy enforcement across microservice benchmarks. \textbf{ZT-GSAD}~\cite{ztgsad}
...
malware-family attribution, we employ JA3 as a \emph{node identity} whose habitual
...
Our contributions focus on the substrate, the gate, and the deployment protocol:
(i) a five-node ZTA chain including a configuration node, making credential reuse
