...
// Schema v4: causal chain source(IP) → config(JA3) → device(TPM) → user → resource.
// One role per column, colour per role; the edges are the hops of the chain.
const NODE_ROLES = {
...
// Extracts the [role, key] chain from the event.
...
featureHead.userData = { title: 'Feature head', type: 'link predictor (MLP)', desc: 'Detects contextual, policy violations and lateral movement novelty.' };
