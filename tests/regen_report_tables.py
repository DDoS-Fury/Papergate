...
* **Panel A** — same standard stream under the default ``TGNConfig()`` (per-cookie keying
  since generator v5, so its TGN row shares Panel B's v4 configuration),
...
* ``docs/paper/generated/tab_baselines.tex`` / ``tab_v3v4.tex`` — ready table-body rows.
...
def run_panel_a_tgn(seeds: list[int]) -> list[dict]:
    """TGN under the SAME protocol as the Panel A baselines: v4, default TGNConfig (per-cookie since generator v5).

    The TGN row runs the baselines' own protocol so the table compares like with like:
    every row uses the same deployable configuration and the same seeds.
    """
    out = []
    for seed in seeds:
        cfg = dataclasses.replace(TGNConfig(), seed=seed)  # default TGNConfig (per-cookie since v5)
        print("\n" + "=" * 80)
        print(f"=== PANEL A: TGN | seed={seed} | default config (per-cookie, v4), "
              f"{cfg.num_events} ev / {cfg.epochs} ep ===")
        print("=" * 80, flush=True)
...
def _emit_panel_b_tex(summary: dict):
    ...
    lines = ["% Generated by tests/regen_report_tables.py — do NOT edit by hand.",
             ...
             "% Columns: Metric & v3 (4 nodes) & v4 (config node) & $\\Delta$ (v4-v3)"]
    for label, key in rows:
        lines.append(f"{label} & {_cell(v3, key)} & {_cell(v4, key)} & "
                     f"{_delta(v4, v3, key)} \\\\")
    lines.append(r"% $^{\dagger}$ = $|\Delta|$ within the multi-seed noise band "
                 r"(not significant).")
...
def _emit_panel_a_tex(tgn: dict, base_summary: dict):
    ...
    metric_rows = [
        ...
        ("Lateral --- AUC", "lat_auc"),
        ("Lateral --- recall @1\\%FPR", "lat_recall_global"),
        ("Aggregate recall @1\\%FPR", "agg_recall_global"),
    ]
    ...
    lines = ["% Generated by tests/regen_report_tables.py — do NOT edit by hand.",
             ...
             "% UNIFIED protocol: TGN row and baselines share the default config (per-cookie since generator v5).",
    ...
    if "A" in args.panels or args.panels == "AB":
        ...
        payload = {
            "meta": _meta(args.seeds,
                          "Panel A: TGN and baselines both default config (per-cookie, v4), "
